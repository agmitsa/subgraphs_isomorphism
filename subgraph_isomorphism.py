...
parser.add_argument('-s', '--simple_enum', action='store_true')
parser.add_argument('-b', '--both_enum', action='store_true')
parser.add_argument("filename1", help="name of input file 1")
parser.add_argument("filename2", help="name of input file 2")
	...
	for e in range(len(Morg[d])):
		if Morg[d][e] != 0:
			...
			for k in range(len(M[d])):
				if k != e:
					M[d][k] = 0	
			#j starts zero out all non-zero elements in the eth column
			for j in range(len(M)):
				if j != d:
					M[j][e] = 0						
			if d == len(A)-1:
				product = cMatrix(M, B)
				transpose = transposeMatrix(product)
				C = cMatrix(M, transpose)
...
a_matrix, c1 = adjencymatrix(args.filename1)
b_matrix, c2 = adjencymatrix(args.filename2)
m_matrix = mMatrix(a_matrix, b_matrix)

if args.both_enum: 
	simple_enumeration(A, B, M, 0)
	refined_enumeration(a_matrix, b_matrix, m_matrix, 0)	
	
if not args.simple_enum: 
	ret = refined_enumeration(a_matrix, b_matrix, m_matrix, 0)
	
if args.simple_enum: 
	ret = simple_enumeration(a_matrix, b_matrix, m_matrix, 0)
...
print('number of calls', recursions)
